...
export function addHealingPoints(actor) {
  const data = {};
  renderTemplate(
    "systems/usr-modern/templates/helpers/heal-dialog.hbs",
    data
  ).then((content) => {
    let d = new Dialog({
      ...
      buttons: {
        heal: {
          ...
          callback: (html) => {
            ...
            const health = actor.system.health;
            health.hp =
              (health.hp ?? 0) + parseInt(html.find("#add-hp")[0].value ?? "0");
            if (health.hp > 0) {
...
export function addDamage(actor) {
  ...
  renderTemplate(
    "systems/usr-modern/templates/helpers/damage-dialog.hbs",
    data
  ).then((content) => {
    let d = new Dialog({
...
function resistDamage(amount, type, actor) {
  ...
  new Roll(`${amount}d10cs<=${resist}`).evaluate().then((roll) => {
    console.log(roll);
